<template>
  <el-drawer title="Timeline 配置" size="50%" :visible.sync="visible" append-to-body :before-close="beforeClose">
    <el-form ref="form" :model="formData" :rules="rules" label-position="top" size="small" label-suffix="：">
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="指定节点排序方向" prop="reverse">
            <el-switch v-model="formData.reverse" active-text="倒序" inactive-text="正序" />
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-form-item label="节点配置">
        ...
        <draggable :list="formData.options" handle=".sortable__handle" ghost-class="sortable__ghost">
          <el-row :gutter="20" v-for="(child, index) in formData.options" :key="index" class="sortable">
            ...
            <el-col :span="8">
              <el-date-picker style="width: 100%;" format="timestamp" size="mini" v-model="child.timestamp" />
            </el-col>
            <el-col :span="8">
              <el-input size="mini" v-model="child.content" />
            </el-col>
            <el-col :span="4">
